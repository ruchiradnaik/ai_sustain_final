...
LOCATIONS = {
    ...
    "Pune Railway Station": (18.5286, 73.8740),
    "Katraj": (18.4497, 73.8686)
}
...
def find_stations_in_path(orig_lat, orig_lon, dest_lat, dest_lon, buffer_km=1.5):
    ...
    for _, stop in all_stops.iterrows():
        ...
        if min_dist_to_path <= buffer_km and detour_factor <= 1.2:  # On-path check
            nearby.append(stop)
    ...
    if not df_nearby.empty:
        ...
        df_nearby = df_nearby.sort_values('dist_from_orig').head(7)  # Limit to 7
    return df_nearby
...
def assemble_route(orig, dest, stations):
    ...
    if stations.empty or total_direct < 2:
        ...
        return [{
            ...
            "points": points, "dist_km": dist_km, "time_min": time_min, "fare": 0
        }]
    ...
    for _, stop in stations.iterrows():
        ...
        if mode == 'Walk' and dist_km > 2:
            mode = stop['Type']  # Fallback to available type
        
        # Skip if detour too big (double-check)
    ...
    if final_dist > 0:
        chain.append({
            "from": prev_name, "to": "Destination", "mode": final_mode if final_dist <= 2 else 'Bus',  # Override long final walk
...
if run_button:
    ...
    total_dist, total_fare, total_time = 0, 0, 0
    ...
    for i, seg in enumerate(chain):
        st.write(f"Segment {i+1}: {seg['from']} → {seg['to']} via {seg['mode']} ({seg['dist_km']:.2f} km, ₹{seg['fare']}, ~{seg['time_min']:.0f} min)")
    st.write(f"**Total**: {total_dist:.2f} km, ₹{total_fare}, ~{total_time:.0f} min (Eco-savings: Prioritized low-emission modes)")
