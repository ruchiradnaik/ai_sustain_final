...
# Find stations (improved with detour filter)
def find_stations_in_path(orig_lat, orig_lon, dest_lat, dest_lon, buffer_km=1.5):
    ...
    bus_stops = bus_df[["Start Stop", "Start_Lat", "Start_Lon"]].rename(
        columns={"Start Stop": "Stop", "Start_Lat": "Lat", "Start_Lon": "Lon"})
    bus_stops["Type"] = "Bus"
    metro_stops = metro_df[["Start_Station", "Start_Latitude", "Start_Longitude"]].rename(
        columns={"Start_Station": "Stop", "Start_Latitude": "Lat", "Start_Longitude": "Lon"})
    ...
    if not df_nearby.empty:
        df_nearby['dist_from_orig'] = df_nearby.apply(
            lambda r: haversine(orig_lon, orig_lat, r['Lon'], r['Lat']), axis=1)
...
# Assemble route with CO2
def assemble_route(orig, dest, stations):
    total_direct = haversine(orig[1], orig[0], dest[1], dest[0])
    if stations.empty or total_direct < 2:
        mode = 'Walk'
        points, dist_km, time_min = get_route_geometry(orig[0], orig[1], dest[0], dest[1], mode, MAPBOX_API_KEY)
        return [{
            "from": orig[2], "to": dest[2], "mode": mode,
            "points": points, "dist_km": dist_km, "time_min": time_min,
            "fare": 0, "co2_g": dist_km * EMISSION_RATES.get(mode, 0)
        }]
    
    chain = []
    prev_lat, prev_lon, prev_name = orig[0], orig[1], "Origin"
    
    for _, stop in stations.iterrows():
        dist_end = haversine(stop['Lon'], stop['Lat'], dest[1], dest[0])
        co2_save = 50 if stop['Type'] == 'Metro' else 20
        mode = predict_mode(0, dist_end, 10, 10, co2_save)
        if mode == 'Mixed': 
            mode = stop['Type']
        
        points, dist_km, time_min = get_route_geometry(prev_lat, prev_lon, stop['Lat'], stop['Lon'], mode, MAPBOX_API_KEY)
        ...
        # Skip detours
        if (haversine(orig[1], orig[0], stop['Lon'], stop['Lat']) + dist_end) / total_direct > 1.2:
            continue
        
        chain.append({
            "from": prev_name, "to": stop['Stop'], "mode": mode,
            "points": points, "dist_km": dist_km, "fare": fare, "time_min": time_min,
            "co2_g": dist_km * EMISSION_RATES.get(mode, 0)
        })
        prev_lat, prev_lon, prev_name = stop['Lat'], stop['Lon'], stop['Stop']
    
    # Final leg
    final_mode = 'Walk'
    points, final_dist, final_time = get_route_geometry(prev_lat, prev_lon, dest[0], dest[1], final_mode, MAPBOX_API_KEY)
    if final_dist > 0:
        chosen_mode = final_mode if final_dist <= 2 else 'Bus'
        chain.append({
            "from": prev_name, "to": "Destination", "mode": chosen_mode,
            "points": points, "dist_km": final_dist, "fare": 0, "time_min": final_time,
            "co2_g": final_dist * EMISSION_RATES.get(chosen_mode, 0)
        })
...
if run_button:
    ...
    chain = assemble_route(orig, dest, stations)
    ...
    for seg in chain:
        folium.PolyLine(
            seg['points'],
            color=color_map.get(seg["mode"], "black"), weight=5,
            tooltip=f"{seg['mode']} ({seg['dist_km']:.2f} km)"
        ).add_to(m)
    ...
    st.subheader("Recommended Eco-Friendly Route")
    for i, seg in enumerate(chain):
        st.write(
            f"Segment {i+1}: {seg['from']} → {seg['to']} via {seg['mode']} "
            f"({seg['dist_km']:.2f} km, ₹{seg['fare']}, ~{seg['time_min']:.0f} min, {seg['co2_g']:.0f}g CO2)"
        )
    st.write(
        f"**Total**: {total_dist:.2f} km, ₹{total_fare}, ~{total_time:.0f} min, {total_co2:.0f}g CO2 "
        f"(Eco-savings: Prioritized low-emission modes)"
    )
